...
## 60. Audio, first slice: the algorithm crosses to Kotlin — 9 Sep 2026

§6A says to build BirdNET in slices that can be tested, because the last time a screen was
reasoned about carefully without being run it shipped broken (§53). The label bridge and the
prior are pure functions; only the capture screen is not. So the pure half went first, and it
needed no BirdNET files at all.

`core/.../Audio.kt` is now the Kotlin side of spec §4A, against `shared/golden/golden_audio.json`
— the same arrangement the rollup has had since §5.1, generated by
`training/tools/gen_golden_audio.py` and checked in both CI workflows. **135 Kotlin tests pass,
including all seven audio cases on the first run.**

Audio needs its own fixture rather than more cases in the rollup one, because what can drift is
what §4 does not have: which classes end up in a confusion set, and **whether the prior is applied
before or after that set is built**. The second is the one worth guarding. Applying it first is a
two-line mistake, it turns a prior into a silent mask, and nothing in the rollup fixture would
notice. Two of the seven cases exist only to pin it: `a_vagrant_survives_a_hostile_prior` gives a
willow warbler a range likelihood of 1e-6, and asserts it is still in the confusion set and still
carries its raw 0.55 — built prior-first it would have fallen below the margin and vanished.

### The Python side is float32 now, and that is not cosmetic

`audio.py` carried float64 through the conditional distribution while Kotlin holds `Float`. The
vision path already casts to float32 before the rollup; audio did not. Left alone, every golden
value would have disagreed in the seventh decimal — which is harmless right up until a score sits
on the margin, where it decides *set membership* rather than a rounding.

So the Python path now narrows at exactly the points Kotlin does: the score on entry, the
margin product, each renormalised leaf, and the stored raw scores. The float64 accumulation
inside the sum is kept, because Kotlin's `sumOf` does the same. All 416 Python tests still pass.

### Found while generating the fixture: a lone detection is reported at 100%

Not a bug in the port — the port reproduces it faithfully. It is in the spec.

§4A.3 renormalises across the confusion set. When nothing else scores within the margin, the set
is the detection alone, and `s_c / s_c = 1`. Measured on the fixture taxonomy at the strictest
setting the app offers:

| BirdNET says | app says |
|---|---|
| chiffchaff, 0.99 | *Phylloscopus collybita*, 100% |
| chiffchaff, 0.60 | *Phylloscopus collybita*, 100% |
| chiffchaff, 0.26 | *Phylloscopus collybita*, 100% |

**The detection confidence is discarded entirely.** A sound scraping over the 0.25 detection
threshold is presented exactly as a textbook recording, and no certainty setting can refuse it,
because the conditional is 1.0 by construction. For an app whose whole claim is that it says how
sure it is, that is the wrong number in the most visible place — and BirdNET's raw confidences
